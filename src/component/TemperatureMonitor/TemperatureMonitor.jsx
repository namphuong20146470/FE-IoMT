  ...
  useEffect(() => {
    const checkTemperature = async () => {
      try {
        ...
        if (data.nhiet_do1 > 50 && !alertedSensors.nhiet_do1) {
        ...
        } else if (data.nhiet_do1 <= 50 && alertedSensors.nhiet_do1) {
          setAlertedSensors(prevState => ({ ...prevState, nhiet_do1: false }));
        }

        if (data.nhiet_do2 > 50 && !alertedSensors.nhiet_do2) {
        ...
        } else if (data.nhiet_do2 <= 50 && alertedSensors.nhiet_do2) {
          setAlertedSensors(prevState => ({ ...prevState, nhiet_do2: false }));
        }

        if (data.nhiet_do3 > 50 && !alertedSensors.nhiet_do3) {
        ...
        } else if (data.nhiet_do3 <= 50 && alertedSensors.nhiet_do3) {
    ...
    const handleAlert = (sensorName, temperature) => {
      window.alert(`${sensorName} has exceeded 50°C with a temperature of ${temperature}°C`);
    };
    ...
    const sendEmail = (data, sensorsExceeded) => {
      ...
      const templateParams = {
        ...
        subject: subject,
        message: `
          The following temperature sensors have exceeded 50°C:
          ${message}
        `,
      };
